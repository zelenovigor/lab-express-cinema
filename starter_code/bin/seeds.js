...
const Movie = require("../models/Movies"); // importing Movie model

mongoose // connecting to the database
  .connect("mongodb://localhost/igors-database") // name of the database after localhost/....
  .then(
    x =>
      console.log(
        `Connected to Mongo! Database name: "${x.connections[0].name}"`
      ) // we get the name if we connected
  )
...
// importing an array of movies from the Movies.js
Movie.insertMany(movies)
  .then(res => console.log(res))
  .catch(err => console.log(err));

// to check the array of movies tap node bin/seeds.js in terminal
